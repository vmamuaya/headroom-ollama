...
OPENROUTER_API_KEY=*** headroom proxy --backend openrouter

# Ollama Cloud (OpenAI-compatible — set OPENAI_TARGET_API_URL)
export OPENAI_TARGET_API_URL=https://ollama.com/v1
export OPENAI_API_KEY=***  # Ollama Cloud key
headroom proxy
...
See the [Ollama Cloud integration guide](/docs/ollama-cloud) for full setup with Hermes Agent, iGPU, and systemd deployment.
